...
def test_open_flac_file(qtbot, mocker, window, flac_file):
    ...
    qtbot.mouseClick(window.art, Qt.MouseButton.LeftButton)
    window.player.play()
    qtbot.mouseClick(window.art, Qt.MouseButton.LeftButton)
...
def test_open_blank_flac_file(qtbot, mocker, window, blank_flac_file):
    ...
    Qtbot clicks on the file menu then Qt.Key.Key_Down highlights the open file item.
    The mock plugin creates a mock of the QFileDialog window while Qt.Key.Key_Enter executes it.
...
def test_quit_application(qtbot, monkeypatch, window):
    ...
    Qtbot clicks on the file menu and Qt.Key.Key_Down highlights the quit application
...
def test_preferences(qtbot, mocker, window):
    ...
    Qtbot clicks on the edit menu then Qt.Key.Key_Down highlights
    the preferences item. The mock plugin creates a mock of the
    QDialog window while Key_Enter executes it."""
    qtbot.mouseClick(window.edit, Qt.MouseButton.LeftButton)
    qtbot.keyClick(window.edit, Qt.Key.Key_Down)
    mocker.patch.object(QDialog, 'exec_', return_value='accept')
    qtbot.keyClick(window.edit, Qt.Key.Key_Enter)
...
def test_media_library_path(qtbot, mocker, tmpdir, window, config):
    ...
    qtbot.mouseClick(config.dialog_media_library.media_library_button, Qt.MouseButton.LeftButton)
...
def test_playback_options(qtbot, mocker, window, config):
    ...
    qtbot.mouseClick(config.dialog_playback.cover_art_playback, Qt.MouseButton.LeftButton)
    qtbot.mouseClick(config.dialog_playback.cover_art_playback, Qt.MouseButton.LeftButton)
...
def test_view_options(qtbot, mocker, window, config):
    ...
    qtbot.mouseClick(config.dialog_view_options.media_library_view_button, Qt.MouseButton.LeftButton)
    qtbot.mouseClick(config.dialog_view_options.media_library_view_button, Qt.MouseButton.LeftButton)
    qtbot.mouseClick(config.dialog_view_options.playlist_view_button, Qt.MouseButton.LeftButton)
    qtbot.mouseClick(config.dialog_view_options.playlist_view_button, Qt.MouseButton.LeftButton)
    qtbot.mouseClick(config.dialog_view_options.dock_left_side, Qt.MouseButton.LeftButton)
    qtbot.mouseClick(config.dialog_view_options.dock_right_side, Qt.MouseButton.LeftButton)
...
def test_about_dialog(qtbot, mocker, window):
    ...
    Qtbot clicks on the help menu then Qt.Key.Key_Down highlights
    the about item. The mock plugin creates a mock of the
    QMessageBox window while Key_Enter executes it."""
    qtbot.mouseClick(window.help_, Qt.MouseButton.LeftButton)
    qtbot.keyClick(window.help_, Qt.Key.Key_Down)
    mocker.patch.object(QDialog, 'exec_', return_value='finished')
    qtbot.keyClick(window.help_, Qt.Key.Key_Enter)
...
def test_minimalist_view(qtbot, window):
    ...
    Qtbot clicks on the view menu then Qt.Key.Key_Down highlights the
...
def test_media_library(qtbot, window):
    ...
    item and uses Qt.Key.Key_Enter to select it."""
    qtbot.mouseClick(window.view, Qt.MouseButton.LeftButton)
    qtbot.keyClick(window.view, Qt.Key.Key_Down)
    qtbot.keyClick(window.view, Qt.Key.Key_Down)
    qtbot.keyClick(window.view, Qt.Key.Key_Enter)
...
def test_playback_play(qtbot, window):
    ...
    qtbot.mouseClick(window.playback, Qt.MouseButton.LeftButton)
    qtbot.keyClick(window.playback, Qt.Key.Key_Down)
    qtbot.keyClick(window.playback, Qt.Key.Key_Enter)
